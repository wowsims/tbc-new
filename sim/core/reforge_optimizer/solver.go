...
// ---------------------------------------------------------------------------
// LP solve path (mirrors the reference reforger's solveModel / checkCaps).
// ---------------------------------------------------------------------------

// minSolveSeconds is the per-pass time_limit floor for cap-refinement re-solves, so an exhausted
// budget never yields a non-positive HiGHS time_limit.
...
func solveModel(
	...
	updatedVariables := updateReforgeScores(variables, weights)
	model := &lpModel{
		direction:   "maximize",
	...
	anyCapsExceeded, updatedConstraints, updatedWeights, updatedSoftCaps := checkCaps(solution, reforgeCaps, reforgeSoftCaps, updatedVariables, constraints, weights, undershootCaps)
	if !anyCapsExceeded {
	...
	// still gets a valid HiGHS time_limit even once the budget is spent.
	remainingSeconds := math.Max(maxSeconds-elapsedSeconds, minSolveSeconds)
	return solveModel(updatedWeights, reforgeCaps, updatedSoftCaps, updatedVariables, updatedConstraints, undershootCaps, remainingSeconds, signals)
}
...
func checkCaps(
	...
	eachUnitStat(reforgeStatContribution, func(unitStat stats.UnitStat, value float64) {
		...
		if cap != 0 && value > cap && !constraints.has(statName) {
			...
			if getUnitStat(undershootCaps, unitStat) != 0 {
