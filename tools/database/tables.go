...
func LoadAndWriteConsumables(dbHelper *DBHelper, inputsDir string) ([]dbc.Consumable, error) {
	query := `
		SELECT
			...
			WHERE ((i.ClassID = 0 AND i.SubclassID IS NOT 0 AND i.SubclassID IS NOT 8 AND i.SubclassID IS NOT 6) OR (i.ClassID = 7 AND i.SubclassID = 2)) AND ItemEffects is not null AND (s.RequiredLevel >= 50 OR i.ID = 22788 OR i.ID = 13442 OR i.ID = 9224) OR i.ID = 86125 OR i.ID = 22797
			AND s.Display_lang != ''
